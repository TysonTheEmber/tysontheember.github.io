...
export default function EmberCanvas({className}: EmberCanvasProps) {
  ...
  useEffect(() => {
    ...
    const updateParticle = (particle: Particle, time: number) => {
      ...
      } else if (particle.life > particle.maxLife - 150) {
        // Gradual fade with easing
        const remainingLife = particle.maxLife - particle.life;
        const fadeProgress = remainingLife / 150;
        // Apply ease-out curve for more gradual fade
        const easedProgress = Math.pow(fadeProgress, 0.7);
        particle.opacity = easedProgress;
        // Shrink more gradually with easing
        particle.size = particle.baseSize * Math.pow(fadeProgress, 0.5);
      } else {
